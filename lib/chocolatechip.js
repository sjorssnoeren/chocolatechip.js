...
const cookieTableTemplate = (props) => {
  return `
    <table class="choc-table">
      <thead>
        <tr>
          <th>${this._options.translations['cookieTableTemplate.name']}</th>
          <th>${this._options.translations['cookieTableTemplate.supplier']}</th>
          <th>${this._options.translations['cookieTableTemplate.description']}</th>
          <th>${this._options.translations['cookieTableTemplate.expiryDate']}</th>
        </tr>
      </thead>
...
global.ChocolateChip = {
  ...
  eat(options) {
    if(defaultTranslations[options.language] == null){ options.language = 'en_GB' }
  ...
  _didUpdateState(prevState) {
    ...
    Object.keys(cookiePreferences).forEach((key) => {
      ...
      if (span != null) {
        span.innerHTML = checked ? this._options.translations['modalTemplate.enable'] : this._options.translations['modalTemplate.enabled'];
      }
