...
const cookieTableTemplate = (props) => {
  ...
  return `
    <table class="choc-table">
      <thead>
        <tr>
          <th>${translations['cookieTableTemplate.name']}</th>
          <th>${translations['cookieTableTemplate.supplier']}</th>
          <th>${translations['cookieTableTemplate.description']}</th>
          <th>${translations['cookieTableTemplate.expiryDate']}</th>
        </tr>
      </thead>
...
global.ChocolateChip = {
  ...
  _didUpdateState(prevState) {
    ...
    Object.keys(cookiePreferences).forEach((key) => {
      ...
      const { translations } = this._options
      if (span != null) {
        span.innerHTML = checked ? translations['modalTemplate.enable'] : translations['modalTemplate.enabled'];
      }
